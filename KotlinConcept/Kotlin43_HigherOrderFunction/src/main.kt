...
fun main() {
    ...
    testFunc1(200, 100, {x1: Int, x2: Int -> x1 % x2})
    ...
    // ✅ 제일 마지막 배개변수가 함다나 함수를 받은 매개변수이고 람다를 전달할 경우
    // 다음과 같이 작성할 수 있음.
...
// 매개변수로 함수를 받는 함수
// 함수를 받는 매개변수의 타입은 (매개변수 타입) -> 반환 값 타입 형태로 작성.
...
// 함수를 반환하는 함수
// 함수의 반환 타입을 (반환할 함수의 매개변수들의 타입) -> 반환타입 형태로 작성해줌.
fun testFunc2(a1: Int) : (Int, Int) -> Int {
    // 함수 내부에 선언된 변수
    // val a1 = 100

    return fun(x1: Int, x2: Int): Int {
        // 반환되는 함수 안에서 함수를 반환하는 함수의 변수나 매개변수를 사용하는 것이 가능.
        return x1 + x2 + a1
    }
}
...
// 매개 변수를 하나만 가지고 있는 함수랑 람다를 받는 함수
