...
fun main() {
    // 리스트 생성
    // 불변형 리스트
    // 리스트 생성 후의 값의 추가, 수정, 삽입, 삭제 등이 불가능.
    val list1 = listOf(10, 20, 30, 40, 50) // 객체를 관리하는거로 생각.
    println("list1: $list1")
    ...
    // 수정 가능한 리스트 -> ArrayList로 생성.
    // 텅 비어있는 수정 가능한 리스트 생성 -> 나중에 객체를 추가할 수 있음.
    val list4 = mutableListOf<Int>()

    // 생성시 관리할 객체를 지정할 수 있음.
    ...
    // null이 있을 경우
    // null 포함
    val list7 = listOf(10, 20, null, 40, 50, 60)
    // null 포함하지 않음.
    ...
    // 지정한 객체가 앞에서부터 몇 번째에 있는가?
    val index1 = list9.indexOf(20)
    println("index1: $index1")

    // 지정한 객체를 뒤에서부터 찾아 앞에서부터 몇 번째에 있는가?
    val index2 = list9.lastIndexOf(20)
    println("index2: $index2")

    // indexOf, latIndexOf 모두 없는 객체를 지정하면 -1을 반환함.
    val index3 = list9.indexOf(100)
    println("index3 : $index3")

    // 일부를 발췌하여 새로운 리스트를 생성
    // 순서값 1 ~ 3 - 1 까지.
    ...
    val list20 = listOf(10, 20, 30) // 불변형
    val list21 = mutableListOf(10, 20, 30) // 가변형
    ...
    // 삽입할 데이터가 다수라면 -> addAll 사용.
    list21.addAll(3, listOf(2000, 3000, 4000))
    println("list21 : $list21")
    ...
    // list 안에 저장되어 있는 객체를 추출하여 새로운 mutablelist를 생성.
    val list200 = list100.toMutableList()
    list200.add(60)
    println("list200: $list200")

    // mutablelist 안에 저장되어 있는 객체를 추출하여 새로운 list 생성
