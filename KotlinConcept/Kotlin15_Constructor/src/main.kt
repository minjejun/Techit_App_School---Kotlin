...
// 클래스를 통해 객체를 생성하면 무조건 동작해야 하는 코드가 있을 경우 생성자에 만들어줌.
...
fun main() {
    ...
    println("---------------------------")
    // 객체 생성시 호출될 생성자를 선택해줘야 함.
    // 클래스이름(매개변수의 형태)

    // 객체 생성시 값을 전달하지 않았기에 매개변수가 없는 생성자가 호출됨.
    val t2 = TestClass2()
    println("t2 : $t2")
    println("---------------------------")
    // 객체 생성 시 정수값 2개를 전달하였기 때문에 정수형 매개변수 2개가 있는 생성자가 호출됨.
    ...
    // TestClass3은 매개변수가 있는 생성자를 작성해놧음
    // 매개변수가 없는 생성자가 자동으로 추가되지 않음.
    // 오류가 발생.
    // val t5 = TestClass3()
...
// 생성자를 여러개 만들어 제공할 경우 중복되는 코드가 있을 경우 init 코드 블럭에 작성해줌.
...
// this : 객체가 가지고 있는 멤버를 지칭하기 위해 사용.
// 메소드나 생성자의 매개변수 이름과 객체의 멤버변수의 이름이 같을 때 사용/
// 같은 클래스 내의 다른 생성자를 호출하고자할 때 사용.
class TestClass4 {
    var memberA1 = 0
    var memberA2 = 0

    constructor(memberA1: Int, memberA2: Int) {
        // 여기서 사용하는 memberA1 memberA2는 매개변수를 지칭.
        println("memberA1 : $memberA1")
        println("memberA2 : $memberA2")
        // 생성자나 메소드의 매개변수 이름과 멤버 변수의 이름이 같은 경우
        // 멤버 변수를 사용하고자 한다면 this를 사용.
    ...
    // 자바에서 생성자에서 다른 생성자를 부르면 제일 위에 있어야 함.
    // 생성자가 호출되면 지정한 다른 생성자를 먼저 호출하여 코드가 수행되고
    // 다른 생성자의 수행이 끝나면 생성자의 코드가 동작함.
}

// 주 생성자
// 하나의 클래스에서 생성자가 여러개 있을 경우 주 생성자를 지정할 수 있음.
// 주 생성자가 아닌 다른 생성자들은 모두 주 생성자를 무조건 호출해야 함.

// 주 생성자의 매개변수는 모두 멤버 변수로 정의됨.
class TestClass5 {
    // 멤버 변수 정의.
    var a1 = 0
    var a2 = 0
...
// 주 생성자 사용
// constructor에 명시한 변수들은 멤버 변수로 정의가 됨.
// 매개변수 3개를 가지고 있는 생성자가 자동으로 추가됨.
// 이 생성자는 매개변수로 들어오는 값을 멤버 변수에 저장되게해주는 코드가 삽입됨.
// TestClass5와 같은 형태로 변환됨.
// class TestClass6 constructor(var a1: Int, var a2: Int, var a3: Int)
// 주 생성자 정의시 constructor는 생략해도 됨.
...
class TestClass7(var a1: Int, var a2: Int) {
    ...
    // 다른 생성자를 하나 더 만들어서 주 생성자에 정의하지 않은 변수도 값을 넣어줄 수 있어야 함.
    // 이 때, 추가적으로 작성한 생성자는 반드시 주 생성자를 호출해야 함.
    // 생성자(): this() -> 형태로 하여 주 생성자를 반드시 호출해줘야 함.
